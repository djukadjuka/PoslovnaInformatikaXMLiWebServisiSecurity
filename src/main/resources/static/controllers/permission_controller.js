app.controller("permission_controller", function($scope,$http,$cookies,$location,$window,$controller,$filter){

     angular.extend(this, $controller('defaultController', {$scope: $scope}));

     $cookies.put('repositorium','permission');
     ...
     $scope.foo = function(event, obj) {
        ...
        if($scope.state=="edit"){
            $scope.obj={};
            $scope.obj.permission_id=$(event.currentTarget).find(".id").html();
